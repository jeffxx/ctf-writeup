...
plain = bytearray("aaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
#for k in "abcdefghujklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ":
key = bytearray("^&#qD93_")
for count in range(27):
    for tmp in range(255):
        ...
        for i in t2:
            out += encode(i)
        #print out
        result = "OO||O||O|O|||OOOO||||||O|O|||OOO||O|OOOO||O|O|OO|||||OOOO||||O||OO|OO||O||O|O|O|||||OOOOOO|O|O||OOOOOOO||O|||OOOO||OO|OO|||O|OO|O|||O|O|OO|OOOO|OOO|OOO|OOOO||O|OO||||OO||||OOO|O|O||OO||||O||OOO|||O|OO|OO||OO||OOOO|O|"
        offset = 0
        flag = 1
        print len(result) 
        for i in range(8):
            if result[i+count*8] != out[i+count*8]:
                flag = 0
        if flag == 1 :
            print tmp 
            print "is correct!"
